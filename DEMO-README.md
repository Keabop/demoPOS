...
Demo **interactiva y sin backend** del punto de venta (basado en AGROMAR). Corre una
base de datos **Postgres real dentro del navegador** (PGlite + WebAssembly, persistida
...
- `src/lib/demo/` contiene toda la capa demo:
  - `schema.ts` — el esquema REAL de AGROMAR (14 tablas, 13 funciones plpgsql, 4
    triggers de PEPS/caja/crédito) adaptado a PGlite.
...
- La lógica de negocio (descuento PEPS por lotes, movimientos de caja, crédito y
  morosos) **corre de verdad** en la BD del navegador.
